...
\right) := V\left(x(t+h),t+h \right)$ und $c^D(x,u,\kappa):=\int\limits_t^{t+h}c(x,u,\tau)d\tau$. Für die in $[t,t+h]$ auflaufenden Kosten gilt
\begin{align}
...
Dann exisitert eine eindeutige positive semidefinite Lösung $X$ der \ac{ARE} $0=R+XA+A^TX-XBQ^{-1}B^TX$. Weiter ist die Rückkopplung $u^{\ast}(t)=Fx(t)$ mit $F:=-Q^{-1}B^TX$,
