...
\end{exmp}
\section{Formulierung des Optimalsteuerproblems und Lösung}
...
\begin{enumerate}[label=(S\arabic*)]
  ...
  \end{align}
  \item Bestimmen der allgemeinen Lösung von \eqnref{eqn:kap_2_vor_optlsg_s1_x} mit dem Integrationsparameter $c\in\mathbb{R}^{2n}$
  \begin{align}
  ...
  \item Einsetzen von $c(x_a)$ in \eqnref{eqn:kap_2_vor_optlsg_s2} ergibt die optimale (Zustands- bzw. Kozustands-) Trajektorie 
  \begin{align}
...
\begin{figure}[htb]
	\centering
	\input{tikz/dummy}
	\caption{Trajektorien von \exmpref{exmp:kap_2_vor_optlsg_1} mit festem Endzustand}
...
\begin{figure}[htb]
	\centering
	\input{tikz/dummy}
	\caption{Trajektorien von \exmpref{exmp:kap_2_vor_optlsg_2} mit Gewichtung des Endzustandes}
...
\subsection{Numerische Lösung am Beispiel "`Fall C und feste Endzeit $t_b$"'}
...
\begin{enumerate}[label=(S\arabic*)]
  ...
  $\begin{bmatrix} \psi(t_b)\\ \vdots\\ \psi(t_a)
  \end{bmatrix}$ aus 
  \begin{align*}
...
\end{align*}
definiert. Analog zu \secref{sec:kap_2_pr_herl_not_bed} werden die Variationen $\xi(t)=\xi^{\ast}(t)+\epsilon\tilde{\xi}(t)$ mit
...
\begin{itemize}
  ...
  $u(t)=[-a_{max},0]$ gilt, optimal ist, wie bspw. Trajektorie (4) in \figref{fig:kap_2_verbrau_opt_trajs}.
  \begin{figure}[htb]
	\centering
	\input{tikz/dummy}
